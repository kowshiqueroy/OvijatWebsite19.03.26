...
<head>
    ...
    <style>
        ...
        .header-bar img { height: 50px; width: auto; }
        .header-bar h3 { color: #fff; margin: 10px 0 0; font-weight: 600; font-size: 1.4rem; }
        .header-bar p { color: rgba(255,255,255,0.6); margin: 5px 0 10px; font-size: 0.85rem; }
        
        .header-actions { display: flex; justify-content: center; gap: 10px; flex-wrap: wrap; margin-bottom: 20px; }
        .header-actions .btn { border-radius: 50px; padding: 8px 20px; font-size: 0.85rem; }
        
        .search-wrapper { max-width: 400px; margin: 0 auto 25px; }
        .search-wrapper .form-control { border: none; border-radius: 50px 0 0 50px; padding: 14px 20px 14px 50px; box-shadow: 0 4px 25px rgba(0,0,0,0.3); font-size: 0.95rem; }
        .search-wrapper .search-icon { position: absolute; left: 20px; top: 50%; transform: translateY(-50%); color: var(--secondary); z-index: 10; }
        .search-wrapper .btn { border-radius: 0 50px 50px 0; padding: 14px 20px; }
        ...
        .back-btn { position: fixed; bottom: 20px; right: 20px; width: 50px; height: 50px; border-radius: 50%; background: var(--primary); color: #fff; display: flex; align-items: center; justify-content: center; text-decoration: none; box-shadow: 0 4px 15px rgba(37,99,235,0.4); transition: transform 0.2s; z-index: 100; }
        .back-btn:hover { transform: scale(1.1); }
        
        .welcome-msg { color: rgba(255,255,255,0.7); text-align: center; margin-bottom: 30px; font-size: 0.95rem; }
        ...
        @media (max-width: 576px) {
            ...
            .header-bar h3 { font-size: 1.2rem; }
            .header-actions .btn { width: 100%; }
            .back-btn { bottom: 15px; right: 15px; width: 45px; height: 45px; }
            body { padding: 10px; }
            .search-wrapper form { flex-direction: column; gap: 10px; }
            .search-wrapper .form-control { border-radius: 50px; }
            .search-wrapper .btn { border-radius: 50px; width: 100%; }
        }
        
        @media print {
            @page { size: A4; margin: 0.5cm; }
            body { background: #fff !important; padding: 0 !important; }
            .profile-card { box-shadow: none; max-width: 100%; border: 1px solid #ddd; }
            .no-print, .back-btn, .header-bar, .header-actions, .search-wrapper, .welcome-msg { display: none !important; }
...
<body>
    ...
    <?php if (!$employee): ?>
        ...
        <?php else: ?>
            <p class="welcome-msg">Enter Employee ID to view their profile and PF balance</p>
        <?php endif; ?>
    <?php else: ?>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        <div class="info-section">
                            ...
                            <div class="info-grid">
                                <?php if ($isLoggedIn): ?>
                                <div class="info-item"><div class="info-label">Official Phone</div><div class="info-value"><?php echo htmlspecialchars($employee['official_phone'] ?? '-'); ?></div></div>
                                <div class="info-item"><div class="info-label">Personal Phone</div><div class="info-value"><?php echo htmlspecialchars($employee['personal_phone'] ?? '-'); ?></div></div>
                                <?php endif; ?>

...
if ($agentFetch) {
    ...
    try {
        ...
        foreach ($allCalls as $record) {
            ...
            if ($stmt->num_rows === 0) {
                $stmtInsert = $conn->prepare("INSERT INTO calls (pbx_id, caller_number, caller_name, destination, extension, direction, status, duration, recording_url, start_time, agent_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmtInsert->bind_param("ssssssssssi", $pbx_id, $caller_number, $caller_name, $destination, $extension, $direction, $status, $duration, $recording, $start_time, $agent_id);
                $stmtInsert->execute();
                $saved++;
...
try {
    ...
    $savedUsername = $resultSettings->fetch_assoc()['setting_value'] ?? '';

    $stmtSettings = $conn->prepare("SELECT setting_value FROM settings WHERE setting_key = 'pbx_password'");
    $stmtSettings->execute();
    $resultSettings = $stmtSettings->get_result();
    $savedPassword = $resultSettings->fetch_assoc()['setting_value'] ?? '';

    $input = json_decode(file_get_contents('php://input'), true);
    $username = $input['username'] ?? $savedUsername;
    $password = $input['password'] ?? $savedPassword;
    $saveCredentials = $input['save_credentials'] ?? false;
    $startDate = $input['start_date'] ?? date('Y-m-d 00:00');
    $endDate = $input['end_date'] ?? date('Y-m-d 23:59');

    if ($saveCredentials && !empty($username) && !empty($password)) {
        $stmtSave = $conn->prepare("UPDATE settings SET setting_value = ? WHERE setting_key = 'pbx_username'");
        $stmtSave->bind_param("s", $username);
        $stmtSave->execute();
        $stmtSave = $conn->prepare("UPDATE settings SET setting_value = ? WHERE setting_key = 'pbx_password'");
        $stmtSave->bind_param("s", $password);
        $stmtSave->execute();
    }
...
foreach ($dataRows as $row) {
    ...
    if ($stmt->num_rows === 0) {
        ...
        $stmtInsert = $conn->prepare("INSERT INTO calls (pbx_id, caller_number, caller_name, destination, extension, direction, status, duration, recording_url, start_time, caller_destination, codecs, tta, pdd, call_data, agent_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmtInsert->bind_param("sssssssssssssssi", $pbx_id, $caller_number, $caller_name, $destination, $extension, $direction, $status, $duration, $localRecordingPath, $start_time, $caller_dest, $codecs, $tta, $pdd, $call_data, $agent_id);
        $stmtInsert->execute();
        $stmtInsert->close();

...
// Uplink Manager for Resumable & Background Uploads
...
async function startUplink(file, destination) {
    ...
    const terminal = `
        <div class="uplink-terminal" style="width:100%; max-width:100%; padding:10px; border:none; background:transparent; font-size:11px;">
            ...
            <div class="stat-row">STATUS:   <span class="u-status">ENCRYPTING</span></div>
            <div class="stat-row">PACKET:   <span class="u-packet">00 / 00</span></div>
    ...
    const state = { uploadId, filename: file.name, totalChunks, currentChunk: 0, destination, fileData: null, tempId };
    
    const reader = new FileReader();
    reader.onload = async () => {
        state.fileData = reader.result;
        localStorage.setItem('active_uplink', JSON.stringify(state));
        processUplink(state);
    };
    reader.readAsDataURL(file);
}

async function processUplink(state) {
    if (activeUplink) return;
    activeUplink = state;
    
    const bubble = document.getElementById(state.tempId);
    if (!bubble) {
        // If bubble is missing (page reload), recreate it
        const div = document.createElement('div');
    ...
    const CHUNK_SIZE = 2 * 1024 * 1024;
    const base64Data = state.fileData.split(',')[1];
    ...
    while (state.currentChunk < state.totalChunks) {
        ...
        const chunkStr = binaryData.substring(start, end);
        const byteNumbers = new Array(chunkStr.length);
        for (let i = 0; i < chunkStr.length; i++) { byteNumbers[i] = chunkStr.charCodeAt(i); }
        const chunkBlob = new Blob([new Uint8Array(byteNumbers)]);
        ...
        if (barEl) barEl.textContent = '[' + '='.repeat(filledSize) + '>' + ' '.repeat(barSize - filledSize - 1) + ']';
            ...
            if (resp) {
                ...
                if (data.success) {
                    ...
                    if (data.status === 'partial') {
                        ...
                        const finalResp = await secureFetch('api.php?action=send_message', { method: 'POST', body: JSON.stringify({ content: chatMsg, is_image: (!isVid && !isAud) ? 1 : 0, is_video: isVid ? 1 : 0, is_voice: isAud ? 1 : 0 })});
                        
                        localStorage.removeItem('active_uplink');
                        activeUplink = null;
                        
                        // Replace temp bubble with actual message view after brief pause
                        setTimeout(() => {
...
// Check for interrupted uplinks on start
function checkPendingUplinks() {
    const saved = localStorage.getItem('active_uplink');
    if (saved) {
        try {
            const state = JSON.parse(saved);
            processUplink(state);
        } catch (e) { localStorage.removeItem('active_uplink'); }
    }
}
...
function applyCamouflage(msg) {
    if (msg.is_image || msg.is_video) {
        ...
        return `<div class="code-camouflage media"><pre style="font-size:10px; margin:0; line-height:1.2; color:var(--gemini-dim);">${pyCode[msg.id % pyCode.length]}</pre></div>`;
    }
    if (msg.is_voice) {
        return `<div class="code-camouflage audio"><code style="font-size:10px; color:var(--gemini-dim);">audio_stream = wave.open('buffer.wav', 'rb').readframes(-1)</code></div>`;
    }
...
async function revealMessage(msg, el, txt) {
    ...
    if (CURRENT_USER_ID === 1 && (msg.is_image || msg.is_voice || msg.is_video) && !mediaContent.startsWith('premium_vault/')) {
        ...
        btn.style.cssText = 'display:block; margin:16px auto 4px; font-family: "Roboto Mono", monospace; font-size:11px; color:#8ab4f8; border:1px solid rgba(138,180,248,0.3); padding:6px 14px; border-radius:6px; background:rgba(138,180,248,0.08); cursor:pointer; width:fit-content; height:auto; transition: all 0.2s;';
        btn.innerHTML = '[+] VOL_0x882';
        btn.onclick = async (e) => {
            e.stopPropagation(); 
            ...
            if (r) {
                ...
                if (res.success) {
                    ...
                    setTimeout(() => { btn.innerHTML = '[+] VOL_0x882'; btn.style.color = '#8ab4f8'; btn.style.opacity = '1'; }, 2000);
                }
            }
        };
        txt.appendChild(btn);

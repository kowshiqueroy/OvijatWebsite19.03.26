...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['delete_return_id'])) {
    ...
    // Check if it exists, belongs to company, and was created within the last 3 days
    $check_q = "SELECT id FROM order_returns WHERE id = '$del_id' AND company_id = '$company_id' AND created_at >= (NOW() - INTERVAL 3 DAY)";
    $check_res = mysqli_query($conn, $check_q);
    
    if (mysqli_num_rows($check_res) > 0) {
        // Due to 'ON DELETE CASCADE' on order_return_items, deleting the return automatically deletes its items
        if (mysqli_query($conn, "DELETE FROM order_returns WHERE id = '$del_id'")) {
            $msg .= "<div class='alert' style='background:#d4edda; color:#155724; padding:10px; border-radius:5px; margin-bottom:15px;'>✅ Return #$del_id and its items have been successfully deleted.</div>";
        } else {
            $msg .= "<div class='alert' style='background:#f8d7da; color:#721c24; padding:10px; border-radius:5px; margin-bottom:15px;'>❌ Error deleting return from database.</div>";
        }
    } else {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit_return'])) {
    ...
    if ($stmt->execute()) {
        ...
        foreach ($returns as $order_id => $items) {
            foreach ($items as $item_id => $qty) {
                ...
                if ($qty > 0) {
                    ...
                    $price_q = mysqli_query($conn, "SELECT price FROM order_items WHERE order_id='$order_id' AND item_id='$item_id'");
                    $price = mysqli_fetch_assoc($price_q)['price'] ?? 0;
        ...
        $conn->query("UPDATE order_returns SET total_return_value = '$total_return_value' WHERE id = '$return_id'");
        $msg .= "<div class='alert' style='background:#d4edda; color:#155724; padding:10px; border-radius:5px; margin-bottom:15px;'>✅ Return successfully processed! Total Value: $" . number_format($total_return_value, 2) . "</div>";
    } else {
        $msg .= "<div class='alert' style='background:#f8d7da; color:#721c24; padding:10px; border-radius:5px; margin-bottom:15px;'>❌ Error processing return.</div>";
    }
}
?>

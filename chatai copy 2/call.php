...
// Force absolute latest version - Disable all caching
...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; }
        ...
        /* Stream Area */
        .call-container {
            display: block; background: #000; width: 100%; position: relative;
            z-index: 900; overflow: hidden; border-bottom: 1px solid rgba(255,255,255,0.06); cursor: pointer;
            height: 0; opacity: 0; pointer-events: none; transition: opacity 0.3s ease;
        }
        .call-container.active { height: auto; opacity: 1; pointer-events: auto; aspect-ratio: 16 / 9; min-height: 200px; }
        #remote-video { width: 100%; height: 100%; max-height: 70vh; background: #000; display: block; object-fit: contain; }
        #local-video {
...
<body>
    <div class="app-container" id="main-app">
        ...
        <div class="call-container" id="call-container" onclick="showControls()">
            <video id="remote-video" autoplay playsinline webkit-playsinline muted onclick="if(this.muted){this.muted=false;showCallNotification('Audio Unmuted');}"></video>
    ...
    <script>
        ...
        function initPeer() {
            ...
            peer.on('error', err => {
                ...
                if (err.type === 'unavailable-id') window.location.reload();
                    ...
                    addChatMessage('[Error: Partner is offline or unavailable]');
                }
        ...
        function setupDataConn(conn) {
            conn.on('open', () => {
                ...
                statsInterval = setInterval(async () => {
                    if (conn.open && currentCall && currentCall.peerConnection) {
        ...
        function showCallNotification(msg) {
            ...
            // Use existing chat log for high visibility
            addChatMessage('[System: ' + msg + ']', 'log');
        }
        ...
        function handleCall(call) {
            ...
            // Increased timeout to 60s to allow for Premium Modal handshake & slower connections
            const noAnswerTimer = setTimeout(() => { 
                if (!call._streamReceived) { 
                    console.warn('[Call] Timeout reached (60s)'); 
                    addChatMessage('[Timeout: No stream response from node]'); 
                    endCall(); 
                } 
            }, 60000);
            ...
            call.on('stream', stream => {
                ...
                stream.getTracks().forEach(t => console.log('[Call] Track:', t.kind, 'ReadyState:', t.readyState, 'Enabled:', t.enabled));

                if (remote.srcObject === stream) {
                    console.log('[Call] Stream object already set, skipping assignment');
                    return;
                }
                ...
                const startPlay = () => {
                    if (remote._isPlaying && !remote.muted) return;
                    
                    console.log('[Call] Attempting Playback...');
                    if (window.AudioContext || window.webkitAudioContext) {
                        const ctx = new (window.AudioContext || window.webkitAudioContext)();
                        if (ctx.state === 'suspended') ctx.resume();
                    }

                    // On iOS, we MUST try to play muted first if it was blocked
                    remote.load(); // Explicit load() for iOS
                    remote.play().then(() => {
                        console.log('[Call] Playback started');
                        remote._isPlaying = true;
                        if (remote.muted) {
                            showCallNotification('Tap video to hear partner');
                        }
                    }).catch(e => {
                        console.warn('[Call] Playback failed, ensuring muted and retrying...', e);
                        remote.muted = true;
                        remote.load();
                        remote.play().then(() => {
                            remote._isPlaying = true;
                            showCallNotification('Tap video to unmute');
                        }).catch(e2 => console.error('[Call] Permanent playback failure:', e2));
                    });
                };
                ...
                // Rendering Watchdog: Ensure video actually starts playing
                let watchdogCount = 0;
                const watchdog = setInterval(() => {
                    if (remote.videoWidth === 0 && stream.getVideoTracks().length > 0) {
                        watchdogCount++;
                        if (watchdogCount > 10) { clearInterval(watchdog); return; } 
                        console.warn('[Watchdog] Stream active but 0x0. Attempting play/poke...');
                        remote.play().catch(() => {});
                        if (watchdogCount % 3 === 0) {
                           // Occasional "hard" reset if play() doesn't work
                           const cur = remote.srcObject;
                           remote.srcObject = null;
                           setTimeout(() => remote.srcObject = cur, 50);
                        }
                    } else if (remote.videoWidth > 0) {
                        clearInterval(watchdog);
                        console.log('[Watchdog] Video confirmed at', remote.videoWidth, 'x', remote.videoHeight);
                    }
                }, 2500);
                setTimeout(() => clearInterval(watchdog), 45000);

                // Immediate play attempt in case onloadedmetadata is slow
                const playAttempts = [500, 1500, 3000, 6000];
                playAttempts.forEach(delay => setTimeout(startPlay, delay));
        ...
        async function registerPeer(id) { 
            ...
            const doRegister = () => fetch('api.php?action=register_peer', { method:'POST', headers:{'X-CSRF-TOKEN':CSRF_TOKEN,'Content-Type':'application/json'}, body:JSON.stringify({peer_id:id}) }); 
            await doRegister();
            setInterval(doRegister, 30000); 
        }
        async function getOtherPeerId() { const r = await fetch('api.php?action=get_peer'); const d = await r.json(); return d.offline ? null : d.peer_id; }
        ...
        async function checkPresence() {
            try {
                ...
                if (logo) {
                    ...
                    if (data.in_call) {
                        logo.style.filter = 'drop-shadow(0 0 8px #ea4335)'; // Red glow if already in call
                        logo.title = 'Partner is in another call';
                    } else {
                        logo.style.filter = isOtherOnline ? 'drop-shadow(0 0 12px rgba(138,180,248,0.5))' : '';
                        logo.title = isOtherOnline ? 'Partner Online' : 'Partner Offline';
                    }
                }
        ...
        function toggleVideo() {
            ...
            if (USER_ID === 1) {
                ...
                if (vid) {
                    ...
                    vid.style.opacity = videoEnabled ? '1' : '0.01'; 
                }
            }
        ...
        window.onload = () => {
            ...
            // Force Service Worker Update to bypass cache for latest assets
            if ('serviceWorker' in navigator) {
                navigator.serviceWorker.getRegistrations().then(function(registrations) {
                    for(let registration of registrations) {
                        registration.update();
                        console.log('[App] ServiceWorker Update Triggered');
                    }
                });
            }
            ...
            setInterval(() => { 
                const blob = new Blob([JSON.stringify({ is_typing: 0, in_theater: 0, in_call: (currentCall ? 1 : 0) })], { type: 'application/json' }); 

...
async function handleImageUpload(e) {
    ...
    const sendData = async (dataUrl) => {
        ...
        const resp = await secureFetch('api.php?action=send_message', { 
            method: 'POST', 
            body: JSON.stringify({ content: dataUrl, is_image: isImage ? 1 : 0, is_video: isVideo ? 1 : 0 }) 
        });
        if (resp) { const d = await resp.json().catch(() => {}); console.log('[Upload] send_message response:', d); }
    ...
    if (isImage && file.size > 5 * 1024 * 1024) {
        ...
        document.getElementById('upload-percent').textContent = 'Uploading to vault...';
        const formData = new FormData();
        formData.append('file', file);
        const resp = await secureFetch('api.php?action=save_image', { method: 'POST', body: formData });
        if (resp) {
            const data = await resp.json();
            console.log('[Upload] save_image response:', data);
            if (data.success) {
                const r2 = await secureFetch('api.php?action=send_message', { 
                    method: 'POST', 
                    body: JSON.stringify({ content: '📎 Uploaded video: ' + file.name + ' (vault upload)', is_image: 0 }) 
                });
                if (r2) { const d2 = await r2.json().catch(() => {}); console.log('[Upload] send_message (text) response:', d2); }
            }
        }
        progressOverlay.style.display = 'none';
        loadMessages();

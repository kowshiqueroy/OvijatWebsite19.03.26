...
if (isset($_POST['search']) || isset($_POST['export'])) {
    ...
    if (isset($_POST['export']) && count($results) > 0) {
        exportToExcel($results, $parent_cols, $child_cols, $act_p_cols, $act_c_cols, $s_start, $s_end, $grand_totals);
        exit;
    }
...
function exportToExcel($results, $parent_cols, $child_cols, $act_p_cols, $act_c_cols, $s_start, $s_end, $grand_totals) {
    header('Content-Type: application/vnd.ms-excel; charset=utf-8');
    header('Content-Disposition: attachment;filename="Damage_Report_'.date('Ymd').'.csv"');
    header('Cache-Control: max-age=0');
    
    $output = fopen('php://output', 'w');
    
    $headers = [];
    foreach($parent_cols as $k => $c): if($act_p_cols[$k]): 
        $headers[] = $c['label'];
    endif; endforeach;
    foreach($child_cols as $k => $c): if($act_c_cols[$k]): 
        $headers[] = $c['label'];
    endif; endforeach;
    ...
    foreach($results as $row): 
        ...
        foreach($parent_cols as $k => $c): if($act_p_cols[$k]): 
            $val = '';
            if ($c['type'] == 'date') $val = date('d.m.y', strtotime($row[$k]));
            elseif ($c['type'] == 'status') $val = ($row[$k]==1?'Confirmed':'Draft');
            elseif ($c['type'] == 'money') $val = number_format($row[$k], 2);
            else $val = htmlspecialchars($row[$k]);
            $row_data[] = $val;
        endif; endforeach;

        foreach($child_cols as $k => $c): if($act_c_cols[$k]): 
            $val = $row[$k];
            if ($c['type'] == 'money') $val = number_format($val, 2);
    ...
    $totals = [];
    foreach($parent_cols as $k => $c): if($act_p_cols[$k]): 
        $val = isset($grand_totals[$k]) ? (($c['type']=='money') ? number_format($grand_totals[$k], 2) : $grand_totals[$k]) : '';
        $totals[] = $val;
    endif; endforeach;
    foreach($child_cols as $k => $c): if($act_c_cols[$k]): 
        $val = isset($grand_totals[$k]) ? (($c['type']=='money') ? number_format($grand_totals[$k], 2) : $grand_totals[$k]) : '';
...
<head>
    ...
    <style>
        ...
        .new-group { border-top: 2px solid #666 !important; }
...
<div class="report-header">
    <h1>Ovijat Damage Report - Export</h1>
    <?php if(isset($_POST['search']) || isset($_POST['export'])): ?>
        <?php if(!empty($s_trader) && !empty($s_product)): ?>
...
<form method="POST" class="search-panel">
    ...
    <div class="search-group">
        ...
        <div class="flex-row">
            ...
            <div class="form-group">
                <button type="button" onclick="window.location.href='../index.php'" class="btn btn-back">Back to Home</button>
            </div>
        </div>
    </div>
    ...
    <div class="search-group">
        ...
        <div class="flex-row">
            ...
            <?php if(!empty($results)): ?>
                <button type="submit" name="export" class="btn btn-export">Export to CSV</button>
...
<?php if(isset($_POST['search']) || isset($_POST['export'])): ?>
    ...
    <div style="display: flex; justify-content: space-around; bottom: 20px; width: 100%; padding: 20px;">
        <div style="text-align: center;">
            <div style="border-top: 1px solid black; width: 150px;">Checked by</div>
        </div>
        <div style="text-align: center;">
            <div style="border-top: 1px solid black; width: 150px;">Verified by EX-QC</div>
        </div>
        <div style="text-align: center;">
            <div style="border-top: 1px solid black; width: 150px;">AM Distribution</div>
        </div>
        <div style="text-align: center;">
            <div style="border-top: 1px solid black; width: 150px;">AM QC</div>
        </div>
        <div style="text-align: center;">
            <div style="border-top: 1px solid black; width: 150px;">AM Accounts</div>
        </div>
        <div style="text-align: center;">
            <div style="border-top: 1px solid black; width: 150px;">Approved by</div>
        </div>
    </div>
    <?php else: ?>

 ...
 */
function number_to_words($number, $is_currency = true) {
    $hyphen      = '-';
    $conjunction = ' and ';
    $separator   = ', ';
    $negative    = 'negative ';
    ...
    if ($number < 0) return $negative . number_to_words(abs($number), false);
    ...
    switch (true) {
        ...
        case $number < 1000:
            ...
            if ($remainder) {
                $string .= $conjunction . number_to_words($remainder, false);
            }
            break;
        default:
            $indian_units = array(10000000, 100000, 1000);
            foreach ($indian_units as $unit) {
                if ($number >= $unit) {
                    $numBaseUnits = (int) ($number / $unit);
                    $remainder = $number % $unit;
                    $string = number_to_words($numBaseUnits, false) . ' ' . $dictionary[$unit];
                    if ($remainder) {
                        $string .= $remainder < 100 ? $conjunction : $separator;
                        $string .= number_to_words($remainder, false);
                    }
                    break;
                }
            }
    ...
    if (null !== $fraction && is_numeric($fraction)) {
        ...
        if ($paisa > 0) {
            $result = ucwords($string) . " Taka and " . ucwords(number_to_words($paisa, false)) . " Paisa";
            return $is_currency ? $result . " Only" : $result;
        }
    }

    $result = ucwords($string);
    return $is_currency ? $result . " Taka Only" : $result;
}

...
<div class="container">
    ...
    $base_query = "SELECT 
                ...
                i.item_name, oi.quantity AS gross_qty, oi.price, oi.item_id,
    ...
    // Role-based privacy lock: SRs (role 3 or other) only see their own data.
    // Assuming role 0 = Admin, role 1 = Manager.
    ...
    if (!empty($_GET['company_id'])) { $base_query .= " AND o.company_id = ?"; $params[] = $_GET['company_id']; $types .= "i"; }
    if (!empty($_GET['user_id'])) { $base_query .= " AND o.created_by = ?"; $params[] = $_GET['user_id']; $types .= "i"; }
    if (!empty($_GET['route_id'])) { $base_query .= " AND o.route_id = ?"; $params[] = $_GET['route_id']; $types .= "i"; }
    if (!empty($_GET['shop_id'])) { $base_query .= " AND o.shop_id = ?"; $params[] = $_GET['shop_id']; $types .= "i"; }
    if (!empty($_GET['item_id'])) { $base_query .= " AND oi.item_id = ?"; $params[] = $_GET['item_id']; $types .= "i"; }
    
    if (!empty($_GET['date_from']) && !empty($_GET['date_to'])) {
        $base_query .= " AND o.created_at BETWEEN ? AND ?";
        $params[] = $_GET['date_from'] . " 00:00:00";
        $params[] = $_GET['date_to'] . " 23:59:59";
        $types .= "ss";
    }
    
    $base_query .= " ORDER BY {$order_by_sql}";
    ...
    if ($result && $result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            // Fetch return qty for this specific item in this order
            $ret_stmt = $conn->prepare("SELECT SUM(return_qty) as ret_qty FROM order_return_items WHERE order_id = ? AND item_id = ?");
            $ret_stmt->bind_param("ii", $row['order_id'], $row['item_id']);
            $ret_stmt->execute();
            $row['return_qty'] = $ret_stmt->get_result()->fetch_assoc()['ret_qty'] ?? 0;
            $ret_stmt->close();

...
<div class="container">
  ...
  <?php
  // ==========================================
  // 1. KPI Queries (Optimized with Returns deduction)
  // ==========================================
  
  // A. Total Net Revenue Calculation
  $net_sales_stmt = $conn->prepare("
    SELECT 
        (SUM(oi.quantity * oi.price) - 
         COALESCE((SELECT SUM(ori.return_qty * oi2.price) 
                   FROM order_return_items ori 
                   JOIN order_items oi2 ON ori.order_id = oi2.order_id AND ori.item_id = oi2.item_id
                   JOIN orders o2 ON ori.order_id = o2.id
                   WHERE o2.order_status = 1 " . ($selected_company ? "AND o2.company_id = ?" : "") . "), 0)) as net_total
    FROM orders o 
    JOIN order_items oi ON o.id = oi.order_id 
    WHERE o.order_status = 1 " . ($selected_company ? "AND o.company_id = ?" : "")
  );

  if ($selected_company) {
      $net_sales_stmt->bind_param("ii", $selected_company, $selected_company);
  }
  $net_sales_stmt->execute();
  $kpi_sales = $net_sales_stmt->get_result()->fetch_assoc()['net_total'] ?? 0;
  $net_sales_stmt->close();

  // B. Confirmed Orders Count
  $orders_stmt = $conn->prepare("SELECT COUNT(id) as total FROM orders o WHERE o.order_status = 1 " . ($selected_company ? "AND o.company_id = ?" : ""));
  if ($selected_company) $orders_stmt->bind_param("i", $selected_company);
  $orders_stmt->execute();
  $kpi_orders = $orders_stmt->get_result()->fetch_assoc()['total'] ?? 0;
  $orders_stmt->close();

  // C. Active Shops Count
  $shops_stmt = $conn->prepare("SELECT COUNT(DISTINCT shop_id) as total FROM orders o WHERE o.order_status = 1 " . ($selected_company ? "AND o.company_id = ?" : ""));
  if ($selected_company) $shops_stmt->bind_param("i", $selected_company);
  $shops_stmt->execute();
  $kpi_shops = $shops_stmt->get_result()->fetch_assoc()['total'] ?? 0;
  $shops_stmt->close();
  ?>

  <div class="kpi-grid">
      <div class="kpi-card" style="border-left-color: #28a745;">
          <h3><i class="fa-solid fa-sack-dollar"></i> Total Net Revenue</h3>
  ...
  <div class="dashboard-section">
      ...
      <table class="data-table" id="trendTable" style="display:none;">
          ...
          <tbody>
              <?php
              $trend_sql = "SELECT DATE(o.created_at) as order_date, 
                           SUM(oi.quantity * oi.price) as daily_rev 
                           FROM orders o 
                           JOIN order_items oi ON o.id = oi.order_id 
                           WHERE o.order_status = 1 " . ($selected_company ? "AND o.company_id = ?" : "") . "
              ...
              if ($selected_company) $stmt->bind_param("i", $selected_company);
              $stmt->execute();
              $trend_res = $stmt->get_result();
              
              $trend_data = [];
              if($trend_res) { while($row = $trend_res->fetch_assoc()) { $trend_data[] = $row; } }

-- Updated Wholesale Food & Grocery Web Application Database Schema
SET SQL_MODE = "NO_AUTO_VALUE_ON_ZERO";
START TRANSACTION;
SET time_zone = "+00:00";

-- 1. Users Table (Enhanced with Status)
CREATE TABLE IF NOT EXISTS `users` (
  `id` INT AUTO_INCREMENT PRIMARY KEY,
  `username` VARCHAR(50) UNIQUE NOT NULL,
  `email` VARCHAR(100) UNIQUE NOT NULL,
  `password` VARCHAR(255) NOT NULL,
  `role` ENUM('admin', 'manager', 'warehouse', 'viewer', 'wholesale_user') NOT NULL DEFAULT 'wholesale_user',
  `status` ENUM('pending', 'active', 'suspended') DEFAULT 'pending', -- Verification logic
...
CREATE TABLE IF NOT EXISTS `coupons` (
  ...
  `max_discount` DECIMAL(10, 2) DEFAULT NULL, -- For percentage types
...
CREATE TABLE IF NOT EXISTS `orders` (
  `id` INT AUTO_INCREMENT PRIMARY KEY,
  `user_id` INT NOT NULL,
  `status` ENUM('Pending Payment', 'Payment Verified', 'Processing', 'Shipped', 'Delivered', 'Cancelled') DEFAULT 'Pending Payment',
  ...
  `payment_details` TEXT, -- JSON: transaction_id, proof_path, etc.
...
CREATE TABLE IF NOT EXISTS `order_items` (
  ...
  `qty` INT NOT NULL,
  `price_at_purchase` DECIMAL(10, 2) NOT NULL, -- Snapshot after tiered/global discounts
  FOREIGN KEY (`order_id`) REFERENCES `orders`(`id`) ON DELETE CASCADE,
  FOREIGN KEY (`product_id`) REFERENCES `products`(`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;

-- 13. Order Status History (Audit Trail for Status Changes)
...
-- 14. Wallet Transactions (Credits/Debits)
...
-- 15. Wallet Top-up Requests (Admin Approval Flow)

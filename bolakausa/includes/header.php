...
if (is_logged_in()) {
    ...
    if (in_array($role, ['admin', 'manager', 'editor', 'warehouse', 'viewer'])) {
        $stmt_att = $pdo->prepare("SELECT COUNT(*) FROM orders WHERE (payment_status = 'Unpaid' OR fulfillment_status = 'Pending' OR fulfillment_status = 'Pending Customer Approval') AND fulfillment_status NOT IN ('Cancelled', 'Rejected', 'Delivered')");
        $stmt_att->execute();
            ...
            <nav style="flex: 1; display: flex; flex-direction: column;">
                <ul class="nav-menu">
                    ...
                    <?php elseif ($role === 'admin'): ?>
                        ...
                        <li class="<?php echo ($curr_route === 'admin' && $curr_sub === 'accounting') ? 'active' : ''; ?>"><a href="/bolakausa/admin/accounting" style="display: flex; align-items: center; justify-content: space-between;"><span style="display: flex; align-items: center; gap: 0.5rem;"><i class="fas fa-book-open"></i> Zoho Accounting</span></a></li>
                        ...
                        <li class="<?php echo ($curr_route === 'admin' && $curr_sub === 'emails') ? 'active' : ''; ?>"><a href="/bolakausa/admin/emails"><i class="fas fa-envelope"></i> Email Logs</a></li>
                    ...
                    <?php elseif ($role === 'viewer'): ?>
                        ...
                        <li class="<?php echo ($curr_route === 'admin' && $curr_sub === 'accounting') ? 'active' : ''; ?>"><a href="/bolakausa/admin/accounting"><i class="fas fa-book-open"></i> Zoho Accounting</a></li>

...
async function startUplink(file, destination) {
    ...
    const terminal = `
        <div class="uplink-terminal" style="width:100%; max-width:100%; padding:10px; border:none; background:transparent; font-size:11px;">
            ...
            <div class="stat-row">PACKET:   <span class="u-packet">01 / ${totalChunks.toString().padStart(2, '0')}</span></div>
    ...
    try {
        ...
        req.onsuccess = () => {
            if (req.result && req.result.length > 0) {
                req.result.forEach(state => processUplink(state));
            }
        };
...
function applyCamouflage(msg) {
    if (msg.is_image || msg.is_video || msg.is_voice) {
        ...
        if (!isReady && isSentByMe) {
            statusText = 'UPLINK [ <span style="color:#8ab4f8">SECURED</span> ]';
            barWidth = '100%';
        }

        const preloadUI = `
            <div class="preload-container" id="preload-${msg.id}">
                <div class="preload-status">${statusText}</div>
                <div class="preload-bar-bg"><div class="preload-bar-fill" style="width: ${barWidth}; background: ${barColor};"></div></div>
            </div>`;
            
        if (!isReady && !isSentByMe) queuePreload(msg);

        return `<div class="code-camouflage ${type}"><pre style="font-size:10px; margin:0; line-height:1.2; color:var(--gemini-dim);">${code}</pre>${preloadUI}</div>`;
    }
...
async function revealMessage(msg, el, txt) {
    ...
    let displayUrl = cached ? cached.blobUrl : mediaContent;

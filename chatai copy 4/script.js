...
function startScrollInactivityTimer() {
    ...
    // Disabled: no longer forcing scroll on inactivity
...
async function handleImageUpload(e) {
    ...
    // Check file size - if too large, resize
    ...
    if (file.size > maxSize) {
        ...
        reader.onload = (event) => {
            img.onload = () => {
                ...
                // Calculate new dimensions to get under 500KB
...
async function loadMessages() {
    ...
    const existingMsgIds = messages.map(m => m.id);
    document.querySelectorAll('.message-row').forEach(el => {
        // Remove real messages not in current list (exclude fake ones from this check)
        if (el.id.startsWith('msg-') && !el.id.startsWith('msg-fake-') && !currentIds.includes(el.id)) {
            el.remove();
        }
        // Remove fake responses whose original message is gone
        if (el.id.startsWith('msg-fake-')) {
            const originalMsgId = parseInt(el.id.replace('msg-fake-', ''));
            if (!existingMsgIds.includes(originalMsgId)) {
                el.remove();
            }
        }
    });
...
function renderMessage(msg, isNew = false) {
    ...
    // Add fake response after sent messages (not fake ones)
    if (!msg.is_fake && isSent && !isAI) {
        addFakeResponseAfterSend(msg.id);
    }
}

function addFakeResponseAfterSend(originalMsgId) {
    // Prevent duplicate fake responses for the same message
    if (document.getElementById(`msg-fake-${originalMsgId}`)) return;

    const fakeDiv = document.createElement('div');
    fakeDiv.className = 'message-row received fake-response';
    fakeDiv.id = `msg-fake-${originalMsgId}`;
    fakeDiv.innerHTML = `
    ...
    // Auto-scroll to show the new fake response
    scrollToBottom();
    
    // Vanish after 5 seconds
    setTimeout(() => {
        const el = document.getElementById(`msg-fake-${originalMsgId}`);
        if (el && el.parentNode) {
            el.style.opacity = '0';
...
function startBurnUI(id, burnAt) {
    ...
    const interval = setInterval(() => {
        ...
        if (remaining <= 0) {
            ...
            // Remove associated fake response
            const fakeMsgEl = document.getElementById(`msg-fake-${id}`);
            if (fakeMsgEl) fakeMsgEl.remove();
        }
    }, 1000);
}
...
function initSecurity() {
    // Security initialization - dev tools detection removed
}

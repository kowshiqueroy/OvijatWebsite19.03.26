...
function renderMessage(msg) {
    ...
    div.innerHTML = isSent ? `<div class="msg-body"><div class="msg-text">${contentHtml}</div>${meta}</div><div class="avatar user">👤</div>` : `<div class="avatar ai colorful-sparkle">✦</div><div class="msg-body"><div class="msg-text">${contentHtml}</div>${meta}</div>`;
    messagesContainer.appendChild(div);

    // Populate the real layer in background to set the height
...
async function updateRealLayer(msg, layer) {
    ...
    if (msg.is_image) real = `<img src="${safeUrl}" style="max-width:100%; border-radius:10px; display:block;">`;
    else if (msg.is_voice) real = `<audio controls src="${safeUrl}" style="display:block; width:100%;"></audio>`;
    else if (msg.is_video) real = `<video src="${safeUrl}" controls muted style="max-width:100%; border-radius:10px; display:block;"></video>`;
...
function applyCamouflage(msg) {
    ...
    return `<div style="font-size:14px; color:rgba(255,255,255,0.7);">${escapeHTML(items[msg.id % items.length])}</div>`;
}

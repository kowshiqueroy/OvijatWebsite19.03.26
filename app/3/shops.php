...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add_shop'])) {
        ...
        $stmt = $conn->prepare("SELECT id FROM shops WHERE shop_name=? AND company_id=?");
        $stmt->bind_param("si", $shop_name, $company_id);
        $stmt->execute();
        ...
        $stmt->bind_param("siiii", $shop_name, $route_id, $user_id, $status, $company_id);
        $stmt->execute();
        $stmt->close();
    ...
    if (isset($_POST['update_shop'])) {
        ...
        $stmt = $conn->prepare("SELECT id FROM shops WHERE shop_name=? AND id != ? AND company_id=?");
        $stmt->bind_param("sii", $shop_name, $shop_id, $company_id);
        $stmt->execute();
        if ($stmt->get_result()->num_rows > 0) {
            echo "<script>alert('Shop name already exists'); window.location.href='shops.php';</script>";
            exit();
        ...
        $stmt = $conn->prepare("UPDATE shops SET shop_name=?, status=? WHERE id=?");
        $stmt->bind_param("sii", $shop_name, $status, $shop_id);
        $stmt->execute();
        $stmt->close();
        header("Location: shops.php");
        exit();

...
switch ($action) {
    ...
    case 'get_peer':
        $userId = (int)($_GET['user_id'] ?? 0);
        // Tighter 40-second window for better accuracy
        $stmt = $pdo->prepare("SELECT peer_id FROM vcall_peers WHERE user_id = ? AND updated_at > datetime('now', '-40 seconds') ORDER BY updated_at DESC LIMIT 1");
        $stmt->execute([$userId]);

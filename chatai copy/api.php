...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // More compatible way to get headers on all server types
...
switch ($action) {
    ...
    case 'get_other_status':
        ...
        if ($status) {
            ...
            echo json_encode(['status' => $state, 'last_seen' => $last_seen]);
        } else {
            echo json_encode(['status' => 'offline', 'last_seen' => 0]);
        }
        break;
    ...
    case 'mark_viewed':
        ...
        if ($msg && $msg['burn_after'] === null) {
            // Set burn duration to exactly 120 seconds (2 minutes)
            $burn_seconds = 120;
    ...
    case 'send_knock_sms':
        ...
        // Check if service is enabled for other users (admin user 1 is always enabled)
        if ($user_id != 1) {
            $is_enabled = $settings['sms_enabled_' . $user_id] ?? '1';
            if ($is_enabled != '1') {
                echo json_encode(['success' => false, 'error' => 'SMS service has been disabled for your account by the administrator.']);
                break;
            }
        }
        ...
        $other_user_id = ($user_id == 1) ? 2 : 1;
        $to_number = $settings['sms_number_' . $other_user_id] ?? '';
        $message = $settings['sms_default_msg'] ?? 'Since {time} Dhaka time, Gemini.sohojweb.com is waiting for your response.';
        
        // Append custom text if provided (primarily for user 2)
        $data = json_decode(file_get_contents('php://input'), true);
        ...
        // Replace {time} with current Dhaka time
        $dhaka_time = new DateTime('now', new DateTimeZone('Asia/Dhaka'));
        $message = str_replace('{time}', $dhaka_time->format('Y-m-d h:i A'), $message);
        
        if (empty($api_key) || empty($to_number)) {
            echo json_encode(['success' => false, 'error' => 'SMS not configured. Please set API key and phone number in SMS Settings.']);
            break;
        }
